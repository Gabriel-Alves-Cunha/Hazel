...
namespace Hazel
{
	...
	void EditorLayer::OnUpdate(TimeStep ts)
	{
		...
		if (FrameBufferSpec spec = m_Framebuffer->GetSpec();
			m_viewPortSize.x > 0.0f && m_viewPortSize.y > 0.0f && //0-sized framebuffer is invalid
			(spec.width != m_viewPortSize.x || spec.height != m_viewPortSize.y))
		{
			m_Framebuffer->Resize((ui32)m_viewPortSize.x, (ui32)m_viewPortSize.y);
			m_CameraController.OnResize(m_viewPortSize.x, m_viewPortSize.y);

			m_ActiveScene->OnViewPortResize((ui32)m_viewPortSize.x, (ui32)m_viewPortSize.y);
		}
		...
		m_Framebuffer->Bind();
		RenderCommand::SetClearColor({ 0.9f, 0.5f, 0.6f, 1.0f });
		RenderCommand::Clear();
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				if (ImGui::MenuItem("Exit"))
					Hazel::Application::Get().CloseWindow();
		...
		ui64 textureID = m_Framebuffer->GetColorAttachmentRendererID();
