#include "hzpch.h"
#include "Hazel/Renderer/OrtographicCameraController.h"
...
namespace Hazel {
	...
	void OrtographicCameraController::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(HZ_BIND_EVENT_FN(OrtographicCameraController::OnMouseScrolled));
		dispatcher.Dispatch<WindowResizeEvent>(HZ_BIND_EVENT_FN(OrtographicCameraController::OnWindowResized));
	}

	void OrtographicCameraController::OnResize(float w, float h)
	{
		m_AspectRatio = w / h;
	...
	void OrtographicCameraController::CalculateView()
	{
